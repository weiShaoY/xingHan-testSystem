...
export class RouteValidator {
    ...
    const checkRoutes = (routes: AppRouteRecord[], parentPath = '') => {
      routes.forEach((route) => {
        ...
        if (route.component && typeof route.component === 'string') {
          ...
          if (componentPath !== RoutesAlias.AdminLayout) {
  ...
  private checkComponents(
    ...
    routes.forEach((route) => {
      ...
      if (parentPath === '' && !hasExternalLink && !isIframe) {
        errors.push(`一级菜单(${routePath}) 缺少 component，必须指向 ${RoutesAlias.AdminLayout}`)
        return
      }
  ...
  private checkNestedIndexComponent(routes: AppRouteRecord[], level = 1): void {
    routes.forEach((route) => {
      ...
      if (level > 1 && route.component === RoutesAlias.AdminLayout) {
  ...
  private logLayoutError(route: AppRouteRecord, level: number): void {
    ...
    console.error(
      ...
      + `  问题: ${level}级菜单不能使用 ${RoutesAlias.AdminLayout} 作为 component\n`
      + `  说明: 只有一级菜单才能使用 ${RoutesAlias.AdminLayout}，二级及以下菜单应该指向具体的组件路径\n`
      + `  当前配置: component: '${RoutesAlias.AdminLayout}'\n`
