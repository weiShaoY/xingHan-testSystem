...
<template>
  ...
  >
    ...
    <template
      v-else-if="examResult"
    >
      <section
        class="rounded-2xl border border-teal-100 bg-linear-to-r from-teal-50 via-white to-cyan-50 px-4 py-5 shadow-[0_8px_20px_rgb(15_23_42/4%)]"
      >
        <div
        ...
        >
          ...
          >
            <div
              class="text-7 text-teal-700 font-700"
            >
              {{ examResult.score }}
            </div>
            ...
            >
              满分 {{ examResult.totalScore }} 分
            </div>
          </div>
        </div>

        <div
          class="mt-5 grid grid-cols-2 divide-x divide-teal-100 rounded-xl bg-white/70 py-3 text-center"
        >
          <div>
            <div
              class="text-4 text-slate-900 font-700"
            >
              {{ scoringRate }}%
            </div>
          ...
          <div>
            <div
              class="text-4 text-slate-900 font-700"
            >
              {{ formatDuration(examResult.durationSeconds) }}
            </div>
      ...
      >
        ...
        >
          ...
          >第 {{ question.sortOrder + 1 }} 题</span>

          <van-tag
            plain
            type="primary"
          >
            {{ questionTypeLabel(question.qusType) }} · {{ question.score }} 分
          </van-tag>
        </div>
        ...
        >
          ...
          >
            <span
              ...
              :class="optionLabelClass(question, optionIndex)"
            ...
            <span
              ...
              class="text-3 text-teal-700 font-600"
            >正确答案</span>

            <span
              v-else-if="isUserOption(question, optionIndex)"
              class="text-3 text-rose-600 font-600"
            >我的选择</span>
          </div>
        </div>

        <div
          class="mt-4 grid grid-cols-1 gap-2 text-3.25 sm:grid-cols-2"
        >
          <div
            class="rounded-xl bg-rose-50 px-3 py-2 text-rose-700"
          >
            我的答案：{{ getOptionText(question, question.userAnswer) }}
          </div>

          <div
            class="rounded-xl bg-teal-50 px-3 py-2 text-teal-800"
          >
            正确答案：{{ getOptionText(question, question.correctAnswer) }}
          </div>
        </div>
