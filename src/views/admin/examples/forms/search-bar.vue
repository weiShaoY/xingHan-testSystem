...
/**
   ...
   * @returns 用户等级选项列表
   */
function fetchLevelOptions(): Promise<OptionItem[]> {
...
/**
   * 获取用户等级数据
   */
async function getLevelOptions(): Promise<void> {
...
/**
   ...
   * @param type 表单类型描述
   */
function createFormHandler(ref: Ref<any>, formData: Record<string, any>, type: string) {
  return {
    reset: () => {
...
/**
   * 批量新增表单项
   */
function batchAddFormItems(): void {
...
/**
   * 重置动态表单项
   */
function resetDynamicItems(): void {
  if (dynamicFormItems.value.length === 0) {
...
<template>
  ...
  >
    ...
    <h2
      class="mb-1 mt-3.5 text-lg font-medium"
    >
      完整示例（默认展开）
    </h2>
    ...
    <ArtSearchBar
      ...
      :label-width="labelWidthAdvanced"
      :label-position="labelPositionAdvanced"
      :span="spanAdvanced"
    ...
    >
      ...
      >
        <ElInput
          ...
          placeholder="我是插槽渲染出来的组件"
        />
      </template>
    </ArtSearchBar>

    <div
      class="art-card p-5 !rounded-lg mt-5"
    >
    ...
    >
      ...
      >
        ...
        <ElButton
          @click="batchAddFormItems"
        >
          批量新增
        </ElButton>

        <ElButton
          @click="resetDynamicItems"
        >
          重置动态项
        </ElButton>
      </ElSpace>
      ...
      >
        ...
        <ElButton
          @click="labelWidthAdvanced = 120"
        >
          修改 label 宽度
        </ElButton>

        <ElButton
          @click="spanAdvanced = 8"
        >
          设置一行显示的组件数
        </ElButton>

        <ElButton
          @click="gutterAdvanced = 50"
        >
          修改 gutter
        </ElButton>

        <ElButton
          @click="labelPositionAdvanced = 'left'"
        >
          label 左对齐
        </ElButton>

        <ElButton
          @click="labelPositionAdvanced = 'right'"
        >
          label 右对齐
        </ElButton>

        <ElButton
          @click="labelPositionAdvanced = 'top'"
        >
          label 顶部对齐
        </ElButton>
      </ElSpace>
