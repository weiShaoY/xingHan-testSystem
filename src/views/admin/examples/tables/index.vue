...
import { CacheInvalidationStrategy, useTable } from '@/hooks/core/useTable'
...
function buildSearchParams(params: typeof searchFormState.value) {
...
/**
   ...
   * @param column 列信息
   */
function handleHeaderClick(column: { label: string, property: string }) {
...
/**
   ...
   * @param sortInfo 排序信息
   */
function handleSortChange(sortInfo: SortInfo) {
...
// 获取事件类型样式
function getEventType(type: string): 'primary' | 'success' | 'warning' | 'info' | 'danger' {
...
/**
   ...
   * @param data 导入的数据数组
   */
function handleImportSuccess(data: Record<string, any>[]) {
...
/**
   ...
   * @param message 日志消息
   */
function addCacheLog(message: string): void {
...
/**
   ...
   */
function updateCacheKeys(key: string, operation: 'add' | 'remove' = 'add'): void {
...
/**
   ...
   * @returns 缓存键摘要
   */
function getCacheKeySummary(key: string): string {
  try {
...
/**
   * 强制刷新缓存信息
   */
function forceRefreshCacheInfo(): void {
...
/**
   * 处理动态列配置命令
   */
function handleColumnCommand(command: string): void {
  switch (command) {
    case 'addColumn': {
      // 新增单个列
      addColumn?.({
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <ElTag
            :type="getUserStatusConfig(row.status).type"
            effect="light"
          >
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              class="flex flex-col gap-1 max-h-50 overflow-y-auto"
            >
              <div
    ...
    >
      ...
      <div
        class="flex flex-wrap gap-2 max-md:flex-col"
      >
        <ElButton
          v-ripple
          @click="refreshData"
