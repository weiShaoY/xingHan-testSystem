...
/**
   ...
   * @param item 菜单项数据
   */
function goPage(item: AppRouteRecord): void {
  closeMenu()
  handleMenuJump(item)
}
...
/**
   ...
   */
function isNavigableRoute(item: AppRouteRecord): boolean {
  return !!(
    !item.meta.isHide
...
/**
   ...
   * @returns 过滤后的菜单项数组
   */
function filterRoutes(items: AppRouteRecord[]): AppRouteRecord[] {
  return items
    .filter((item) => {
      // 如果当前项被隐藏，直接过滤掉
      if (item.meta.isHide) {
...
/**
   ...
   * @returns 是否包含可见的子菜单
   */
function hasChildren(item: AppRouteRecord): boolean {
  if (!item.children || item.children.length === 0) {
    return false
  }
...
/**
   ...
   * @returns 是否为外部链接
   */
function isExternalLink(item: AppRouteRecord): boolean {
...
/**
   ...
   * @returns 唯一的 key
   */
function getUniqueKey(item: AppRouteRecord, index: number): string {
