...
function flattenAndFilterMenuItems(items: AppRouteRecord[], val: string): AppRouteRecord[] {
...
function scrollToHighlightedHistoryItem() {
  nextTick(() => {
...
function searchGoPage(item: AppRouteRecord) {
  showSearchDialog.value = false
  addHistory(item)
  handleMenuJump(item)
  searchVal.value = ''
...
function addHistory(item: AppRouteRecord) {
...
function highlightOnHoverHistory(index: number) {
  if (!isKeyboardNavigating.value && !searchVal.value) {
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <div
          class="dialog-footer box-border flex-c border-t-d pt-4.5 pb-1"
        >
          <div
            class="flex-cc"
          >
            ...
            <span
              class="mr-3.5 text-xs text-g-700"
            >{{ $t('search.selectKeydown') }}</span>
          </div>
