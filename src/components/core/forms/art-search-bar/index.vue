...
function cloneModelValue(value: Record<string, any> | undefined) {
  if (!value) {
    return {
    }
  }
...
function getProps(item: SearchFormItem) {
...
// 获取插槽
function getSlots(item: SearchFormItem) {
  if (!item.slots) {
    return {
    }
  }
...
// 组件
function getComponent(item: SearchFormItem) {
