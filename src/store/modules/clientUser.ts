 ...
 */
const CLIENT_LAST_USER_ID_KEY = `${StorageConfig.LAST_USER_ID_KEY}:client`

/**
 * 客户端用户状态 Store。
 *
 * 负责维护客户端用户登录态、用户信息、语言、锁屏状态、访问令牌、
 * 搜索历史以及与用户相关的持久化状态。
 */
export const useClientUserStore = defineStore(
  'clientUserStore',
  () => {
    /** 当前客户端语言。 */
    ...
    function logOut() {
      ...
      const redirect = currentRoute.name !== 'ClientLogin' ? currentRoute.fullPath : undefined
    ...
    /**
     * 检查客户端是否切换了登录用户，并在用户变化时清空工作台标签页。
     *
